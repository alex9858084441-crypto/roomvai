...
- [x] Политики конфиденциальности (`docs/PRIVACY.md`, `docs/TERMS.md`)
...
Два документа подготовлены в `docs/`:

- **`PRIVACY.md`** — Политика конфиденциальности (GDPR + 152-ФЗ):
  описывает сбор фотографий, учётных данных, аналитики, сторонние сервисы
  (Supabase, Replicate, RevenueCat, PostHog), права пользователя, хранение.
- **`TERMS.md`** — Пользовательское соглашение:
  условия подписки, права на сгенерированные изображения, ограничение
  ответственности, запрещённое использование.

Перед публикацией сторов:

1. Разместите оба документа по публичным URL (напр. `roomvai.com/privacy`
   и `roomvai.com/terms`).
2. Укажите эти URL в метаданных сторов (`mobile/store/`).
3. Проверьте, что экран согласия (ConsentScreen) ссылается на актуальную
   версию политики (`POLICY_VERSION` в `ConsentScreen.tsx`).
